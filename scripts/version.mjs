...
//     → crea la versión siguiente (v2, v3...), la publica y borra versiones
//       antiguas dejando solo las últimas 5.
//
...
function writeVersion(n, descripcion) {
  const fecha = new Date().toISOString().slice(0, 10);
  fs.writeFileSync(VERSION_FILE, JSON.stringify({ version: n, fecha, descripcion }, null, 2) + '\n');
}
...
function guardar(descripcion) {
  const actual = readVersion().version;
  const nueva = actual + 1;
  const desc = descripcion || `Version ${nueva}`;
...
async function restaurar(arg) {
  ...
  const nueva = actual + 1;
  writeVersion(nueva, `Restauración de ${tag}`);
  git('add -A');
  git(`commit -m "v${nueva}: restauración de ${tag}"`);
  git(`tag -a v${nueva} -m "Restauración de ${tag}"`);
  git('push origin main --follow-tags');
  console.log(`\n✅ Listo: la página quedó como en ${tag} (guardado como versión v${nueva}).`);
  console.log('   Publicándose en https://curtiembre-pro.vercel.app (~1 minuto)');
  pruneOldTags(listVersionTags());
}
...
if (comando === 'guardar') guardar(resto.join(' '));
  ...
  console.log('Uso: node scripts/version.mjs [guardar "descripcion" | listar | restaurar [N]]');
}

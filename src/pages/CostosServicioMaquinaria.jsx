...
export default function CostosServicioMaquinaria() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [costosData, lotesData] = await Promise.all([
        CostoIndirecto.filter({ tipo_costo: 'servicio_maquinaria' }),
  ...
  const handleOpenModal = (item = null) => {
    ...
    if (item) {
      setCurrentItem(item);
      setNewItemsList([]); // Editing single item
    } else {
      // Initialize with one empty item in the list
      setNewItemsList([{
        tipo_costo: 'servicio_maquinaria',
        codigo_lote: '',
  ...
  const handleNewItemChange = (index, field, value) => {
    ...
    // If changing lot or date, apply to all for convenience? Maybe not strict requirement, but good UX. 
    // For now keep independent or maybe sync header fields.
    // Let's sync Header fields (Lote, Fecha) if changed on first item to others? No, let's keep simple.
  ...
  const addNewItemRow = () => {
    ...
    setNewItemsList([...newItemsList, {
      tipo_costo: 'servicio_maquinaria',
      codigo_lote: lastItem?.codigo_lote || '', // Copy prev context
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      loadData();
      alert('Servicio(s) de maquinaria guardado(s) con éxito.');
    } catch (error) {
  ...
  const headers = ['Código Lote', 'Fecha', 'Nombre Servicio', 'Cantidad Pieles', 'Valor Unitario', 'Valor Total', 'Subtotal', 'Acciones'];
  ...
  return (
    <div className="p-6">
      <PageHeader
        ...
        description="Gestiona los costos de servicios de maquinaria."
      ...
      <Card id="tabla-imprimible">
        <CardHeader><CardTitle>Listado de Servicios de Maquinaria</CardTitle></CardHeader>
        <CardContent>
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{isEditing ? 'Editar' : 'Nuevo'} Servicio de Maquinaria</DialogTitle>
          </DialogHeader>
            ...
            {isEditing ? (
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div><Label>Fecha de Servicio *</Label><Input type="date" value={currentItem?.fecha_servicio || ''} onChange={e => handleEditChange('fecha_servicio', e.target.value)} required /></div>
                </div>
                <div>
                  ...
                  <Select value={currentItem?.nombre_servicio || ''} onValueChange={v => handleEditChange('nombre_servicio', v)}>
                    ...
                    <SelectContent>
                      <SelectItem value="BOMBOS POR PIEL">BOMBOS POR PIEL</SelectItem>
                      <SelectItem value="DIVIDIDA POR PIEL">DIVIDIDA POR PIEL</SelectItem>
                      <SelectItem value="ESCURRIDA Y REBAJADA POR HOJA">ESCURRIDA Y REBAJADA POR HOJA</SelectItem>
                      <SelectItem value="TEMPLADA TOUGLE POR HOJA">TEMPLADA TOUGLE POR HOJA</SelectItem>
                      <SelectItem value="ABATANADA POR HOJA">ABATANADA POR HOJA</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
        ...
        <DialogContent>
          ...
          {selectedItem && (
            <div className="space-y-3 text-sm">
              ...
              <p><span className="font-semibold">Nombre Servicio:</span> {selectedItem.nombre_servicio}</p>

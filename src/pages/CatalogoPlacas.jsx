...
// Catálogo de Placas: usa ProductoCatalogo con categoria = 'placa', igual que el
// Catálogo de Tipo de Acabado, para mantener el mismo mecanismo de catálogos
// flexibles ya usado en el resto del ERP.
...
export default function CatalogoPlacas() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await ProductoCatalogo.filter({ categoria: 'placa' });
  ...
  const handleSave = async (e) => {
    ...
    if (!isEditing) {
      ...
      if (exists) { alert('Ya existe una placa con este código.'); return; }
    }
    try {
      if (isEditing) {
        await ProductoCatalogo.update(currentItem.id, currentItem);
      } else {
        await ProductoCatalogo.create({ ...currentItem, categoria: 'placa' });
      }
      setShowModal(false);
      loadData();
      alert('Placa guardada con éxito.');
    } catch (error) {
      console.error('Error:', error);
  ...
  const headers = ['Código de Placa', 'Nombre de la Placa', 'Estado', 'Fecha de Creación', 'Última Modificación', 'Acciones'];
  ...
  return (
    <div className="p-6">
      <PageHeader
        ...
        description="Gestión de las placas (tipos de textura/grabado) disponibles para el proceso de pintura."
        actionButton={
          <Button onClick={() => handleOpenModal()} className="bg-emerald-600 hover:bg-emerald-700">
            <Plus className="w-4 h-4 mr-2" /> Nueva Placa
          </Button>
        }
      />
      <Card>
        <CardHeader>
          <CardTitle>Listado de Placas</CardTitle>
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="max-w-xl">
          <DialogHeader><DialogTitle>{isEditing ? 'Editar' : 'Nueva'} Placa</DialogTitle></DialogHeader>
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Código de Placa *</Label><Input value={currentItem?.codigo || ''} onChange={e => setCurrentItem({ ...currentItem, codigo: e.target.value })} required disabled={isEditing} /></div>
              <div><Label>Nombre de la Placa *</Label><Input value={currentItem?.descripcion || ''} onChange={e => setCurrentItem({ ...currentItem, descripcion: e.target.value })} required /></div>
            </div>
            <div>
              <Label>Estado</Label>
            ...
            <div><Label>Descripción</Label><Textarea value={currentItem?.nombre_comercial || ''} onChange={e => setCurrentItem({ ...currentItem, nombre_comercial: e.target.value })} rows={2} /></div>
      ...
      <Dialog open={showDetailModal} onOpenChange={setShowDetailModal}>
        <DialogContent>
          <DialogHeader><DialogTitle>Detalle de Placa</DialogTitle></DialogHeader>
          {selectedItem && (

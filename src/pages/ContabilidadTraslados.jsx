...
export default function ContabilidadTraslados() {
    ...
    const loadData = useCallback(async () => {
        setLoading(true);
        try {
            const data = await CuentaContable.filter({ tipo_cuenta: "traslado_efectivo" });
            setTraslados(data);
    ...
    const handleOpenModal = (item = null) => {
        setIsEditing(!!item);
        setCurrentItem(item || { fecha: new Date().toISOString().split('T')[0], tipo_cuenta: "traslado_efectivo", concepto: "", valor: 0 });
        setShowModal(true);
    };
    ...
    const handleDelete = async (id) => {
        if (confirm("¿Eliminar este traslado?")) {
    ...
    const headers = ["Fecha", "Concepto", "Valor", "Acciones"];
    ...
    return (
        <div className="p-6">
            <PageHeader 
                ...
                description="Registra movimientos de dinero entre cuentas o cajas."
            ...
            <Card id="tabla-imprimible">
                <CardHeader><CardTitle>Historial de Traslados</CardTitle></CardHeader>
                <CardContent>
                    <DataTable headers={headers} data={traslados} renderRow={renderRow} loading={loading} />
                </CardContent>
            </Card>
            <Dialog open={showModal} onOpenChange={setShowModal}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{isEditing ? 'Editar' : 'Nuevo'} Traslado</DialogTitle>
                    </DialogHeader>
                     <form onSubmit={handleSave} className="space-y-4 pt-4">
                        ...
                        <div>
                            ...
                            <Input id="concepto" placeholder="Ej: Traslado de Caja a Banco" value={currentItem?.concepto || ''} onChange={e => setCurrentItem({...currentItem, concepto: e.target.value})} required/>
                        </div>

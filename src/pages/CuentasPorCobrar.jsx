...
import { CuentaPorCobrar } from '@/entities/all';
...
export default function CuentasPorCobrar() {
  ...
  const [filtros, setFiltros] = useState({ cliente: '', estado: 'todos', vencimiento: '' });
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await CuentaPorCobrar.list();
      setCuentas(data);
    } catch (error) {
  ...
  const aplicarFiltros = () => {
    ...
    if (filtros.cliente) {
      filtered = filtered.filter(c => c.cliente_nombre?.toLowerCase().includes(filtros.cliente.toLowerCase()));
    }
  ...
  const headers = ['Cliente', 'Tipo y No. Doc.', 'Fecha Doc.', 'Fecha Venc.', 'Valor Total', 'Valor Cobrado', 'Saldo Pendiente', 'Estado', 'Acciones'];
  
  const renderRow = (cuenta) => (
    <tr key={cuenta.id} className={esVencida(cuenta.fecha_vencimiento) && cuenta.estado !== 'pagada' ? 'bg-red-50' : ''}>
      <td>{cuenta.cliente_nombre} {esVencida(cuenta.fecha_vencimiento) && cuenta.estado !== 'pagada' && <AlertCircle className="w-4 h-4 inline text-red-600 ml-1" />}</td>
      ...
      <td className="text-right text-green-600">{formatCurrency(cuenta.valor_cobrado)}</td>
    ...
    <div className="p-6">
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-4">
            <div><Label>Cliente</Label><Input placeholder="Buscar..." value={filtros.cliente} onChange={e => setFiltros({...filtros, cliente: e.target.value})} /></div>
            <div>
              <Label>Estado</Label>
      ...
      <Dialog open={showDetailModal} onOpenChange={setShowDetailModal}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Detalle de Cuenta por Cobrar</DialogTitle></DialogHeader>
          {selectedCuenta && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 bg-blue-50 p-4 rounded">
                <p><strong>Cliente:</strong> {selectedCuenta.cliente_nombre} (NIT: {selectedCuenta.cliente_nit})</p>
              ...
              <div className="grid grid-cols-3 gap-4 p-4 border rounded">
                ...
                <div><Label>Valor Cobrado</Label><p className="text-lg font-bold text-green-600">{formatCurrency(selectedCuenta.valor_cobrado)}</p></div>
              ...
              <div>
                <h4 className="font-bold mb-2">Historial de Cobros</h4>
                {selectedCuenta.historial_cobros && selectedCuenta.historial_cobros.length > 0 ? (
                  <table className="w-full text-sm border">
                    <thead className="bg-gray-100">
                      <tr>
                        <th className="border p-2">Fecha</th>
                        <th className="border p-2">Valor</th>
                        <th className="border p-2">Forma Cobro</th>
                        <th className="border p-2">Referencia</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedCuenta.historial_cobros.map((cobro, idx) => (
                        <tr key={idx}>
                          <td className="border p-2">{formatDate(cobro.fecha_cobro)}</td>
                          <td className="border p-2 text-right font-bold">{formatCurrency(cobro.valor_cobro)}</td>
                          <td className="border p-2">{cobro.forma_cobro}</td>
                          <td className="border p-2">{cobro.referencia}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ) : (
                  <p className="text-gray-500">Sin cobros registrados</p>
                )}
              </div>

...
export function SeccionProductosQuimicos({ titulo, items, peso }) {
  ...
  const totalHojas = items.reduce((sum, item) => sum + (item.cantidad_pieles || 0), 0);
  const totalPeso = items.reduce((sum, item) => sum + (item.peso_actual || 0), 0);

  if (todosInsumos.length === 0) return null;

  return (
    <div className="border rounded">
      <div className="bg-gray-700 text-white px-3 py-2 font-bold text-center">
        {titulo} - HOJAS: {totalHojas} | PESO: {formatNumber(totalPeso)} KG
      </div>
      <table className="w-full text-xs">
        ...
        <tbody>
          {todosInsumos.map((insumo, idx) => {
            const costoConIva = (insumo.costo_unitario || 0) * (1 + (insumo.iva || 0));
            const dosificacion = insumo.dosificacion || 0;
            ...
            return (
              <tr key={idx}>
                ...
                <td className="border p-1 text-right">{formatCurrency(insumo.costo_unitario)}</td>
                <td className="border p-1 text-right">{formatCurrency((insumo.costo_unitario || 0) * (insumo.iva || 0))}</td>
                ...
                <td className="border p-1 text-right font-bold text-blue-700">{formatNumber(dosificacion)}%</td>
              </tr>
            );
          })}
...
export function SeccionRecurtidoPorColor({ color, items, peso }) {
  ...
  return (
    <div className="border rounded mt-2">
      <div className="bg-purple-700 text-white px-3 py-2 font-bold">
        RECURTIDO - COLOR {color} - HOJAS: {totalHojas} | PESO: {formatNumber(totalPeso)} KG
      </div>
      <table className="w-full text-xs">
        <thead className="bg-gray-200">
          <tr>
